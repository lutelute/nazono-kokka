...
| 入口 | コマンド | 何ができる |
|------|---------|-----------|
| 🔬 **解析ツール**（Ollama 不要・最速で体感） | `./run_tool.sh` | 似ている箇所ハイライト・書庫マップ・クラスタ・近傍検索・重複検出を 6 タブで |
| 📚 **教材ショーケース** | [`docs/index.html`](docs/index.html) をブラウザで開く | ベクトルの仕組みを図解と実データで段階的に（5 教材） |
| ⚖️ **メイン UI**（法律 Q&A は Ollama 必要） | `streamlit run app.py` | 法律 Q&A・精度比較・ベクトル可視化・上記の解析を 1 つの UI で |

> 💡 「まず触って雰囲気を掴みたい」なら 🔬 解析ツール、「仕組みを理解したい」なら 📚 教材ショーケースがおすすめです。
