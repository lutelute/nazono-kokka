...
> **RAG が「意味の似ている／似ていない」をどう捉えるか** を、図解・実データ・触れるツールで体感できるシステム。

架空の国家の法体系を題材にした **RAG (Retrieval-Augmented Generation)** システムです。
AI による司法判断の精度を検証・比較し、さらに埋め込み空間（768 次元）を多角的に解析できます。

> 🌐 **インストール不要で今すぐ試す（GitHub Pages）**
> - 📚 教材ショーケース: **<https://lutelute.github.io/nazono-kokka/>**
> - 🔬 類似度＆解析ツール（ブラウザ内 Python・サーバー不要）: **<https://lutelute.github.io/nazono-kokka/tool/>**

### 4 つの入口（目的に合わせて選ぶ）

| 入口 | 開き方 | 何ができる |
|------|---------|-----------|
| 🌐 **ブラウザ版ツール**（インストール不要） | [lutelute.github.io/nazono-kokka/tool/](https://lutelute.github.io/nazono-kokka/tool/) | ローカル版とほぼ同等の 7 タブ — 類似度行列・3方式ハイライト（文対応/語の寄与/ペアリング）・埋め込み地図 **2D/3D**・法令⇄判例の軸・近傍検索・クラスタ・重複検出（stlite・初回ロード 20〜40 秒） |
| 📚 **教材ショーケース** | [lutelute.github.io/nazono-kokka/](https://lutelute.github.io/nazono-kokka/) | ベクトルの仕組みを図解と実データで段階的に（5 教材＋触れる体験版） |
| 🔬 **解析ツール（ローカル版）**（Ollama 不要） | `./run_tool.sh` | 自由入力テキストの解析も含むフル機能 — ハイライト・書庫マップ・クラスタ・近傍検索・重複検出を 6 タブで |
| ⚖️ **メイン UI**（法律 Q&A は Ollama 必要） | `streamlit run app.py` | 法律 Q&A・精度比較・ベクトル可視化・上記の解析を 1 つの UI で |

> 💡 「まず触って雰囲気を掴みたい」なら 🌐 ブラウザ版（URL を開くだけ）、「仕組みを理解したい」なら 📚 教材ショーケース、
> 「自由なテキストで実験したい」なら 🔬 ローカル版がおすすめです。
